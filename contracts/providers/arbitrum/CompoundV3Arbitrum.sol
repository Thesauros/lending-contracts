...
contract CompoundV3Arbitrum is IProvider {
   ...
   */
  function _getMarketAndAssets(IInterestVault vault)
    private
    view
    returns (CometInterface cMarketV3, address asset/*, address debtAsset*/)
  {
    asset = vault.asset();
    // debtAsset = vault.debtAsset();

    // market == baseToken for Comet
    address market = _providerManager.getProtocolToken(getProviderName(), asset);
