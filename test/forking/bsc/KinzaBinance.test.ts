...
describe('KinzaBinance', async () => {
  ...
  let kinzaProvider: KinzaBinance;
  ...
  beforeEach(async () => {
    ...
    kinzaProvider = await new KinzaBinance__factory(deployer).deploy();
    ...
    wbnbRebalancer = await deployVault(
      ...
      'Rebalance tWBNB',
      'rtWBNB',
      [await kinzaProvider.getAddress()]
    );
  ...
  describe('getProviderName', async () => {
    it('Should get the provider name', async () => {
      expect(await kinzaProvider.getProviderName()).to.equal('Kinza_Binance');
    });
  });
  ...
  describe('interest rates', async () => {
    it('Should get interest rates', async () => {
      await deposit(alice, wbnbRebalancer, DEPOSIT_AMOUNT);
      let depositRate = await kinzaProvider.getDepositRateFor(
