...
describe("RadiantV2Arbitrum", async () => {
  ...
  beforeEach(async () => {
    ...
    await mainAsset
      .connect(deployer)
      .deposit({ value: depositAmount + initAmount });
    await mainAsset.connect(alice).deposit({ value: depositAmount });
    await mainAsset.connect(bob).deposit({ value: depositAmount });
